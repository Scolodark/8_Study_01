...
public class GameManager : MonoBehaviour
{
    ...
    //플레이어가 사망 했을때 현재 점수가 랭크로 부터 몇등인지 가져오는 함수
    private int GetRank()
    {
    ...
    private void initGameoverMenu()
    {
        ...
        {
            //objGameoverMenu.SetActive(true);//켜지게됨
    ...
    {
        //랭크에 들어갔다면 name과 score를 ranklist안에 저장해주고 11등은 삭제
        if (rank != -1)
        {
        ...
        //오버라이드(완전 똑같이 하되 내가 활용할때 다르게 이용 하는 것), 오버로드(함수명은 같지만 내부 변수가 다를때) 

        SceneManager.LoadSceneAsync((int)SceneNums.MainScene);//메인신 이동
        
        
        //랭크에 들지 못했다면 다른Scene으로 이동(Main Scene)
    }
    ...
    public void GameOver()//플레이어가 사망했을 때 동작
    {
        rank = GetRank();//rank에 들지 않았다면 -1
        //rank에 들었다면 이름과 점수를 기록할수있는 메뉴
        if(rank != -1)
        {
